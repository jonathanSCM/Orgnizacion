...
- El `prisma db push` del arranque corre con `--accept-data-loss`: si en algún momento se cambia `prisma/schema.prisma` de una forma que implique pérdida de datos en una tabla puntual (ej. una columna nueva obligatoria en una tabla que ya tiene filas, o borrar una columna con datos), el contenedor va a aplicar el cambio igual en vez de rechazarlo — necesario porque el arranque es no interactivo y sin este flag el contenedor queda en loop de reinicio esperando una confirmación que nunca llega (fue justo lo que pasó con la tabla `ApiToken` al pasar de token hasheado a texto plano). El costo es que ese tipo de cambios ya no avisan antes de aplicarse: si vas a tocar el schema de una tabla con datos importantes, pensalo dos veces o hacé un backup manual antes de ese deploy puntual.
